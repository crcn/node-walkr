...
module.exports = function(_source, _destination) {
	...
	_self.start = function(callback) {
		...
		_walk(_source, _destination, callback);
		...
		if(search instanceof RegExp) {
			...
			fn = {
				test: function(options) {
					...
					return search.test(options.name);
	...
	function _filter(options, next) {
		...
		function filterNext(index) {
			...
			var throwAway = filter.test(options, function(err, result) {
				
				//arguments? stop the filter
				return arguments.length ? next(false) : filterNext(index + 1);
...
module.exports.copy = function(options, next) {
	...
	if(options.stat.isDirectory() || !options.destination) return false;
